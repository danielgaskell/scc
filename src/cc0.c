...
/* _itoa2 : always use our own inbuilt one. We don't want to suck in all
   of sscanf */
...
{
	if (bufleft == 0) {
		bufleft = rread(fdi, buffer, BLOCK);
		if (bufleft == 0)
			return EOF;
		bufptr = buffer;
	}
...
static void outflush(void)
{
	...
	if (len && write(fdo, outbuf, len) != len)
		error("I/O");
}
...
int main(int argc, char *argv[])
{
	...
	symtab = "$symtab";
    if (argv[1] != NULL) {
        fdi = open(argv[1], O_RDONLY | O_BINARY, 0600);
        if (fdi == -1)
            fatal("error opening input file");
        fdo = open("$stream1", O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0600);
        if (fdo == -1)
            fatal("error opening output stream");
    } else {
        fatal("usage: cc0 filename.c");
    }
	keywords();
	do {
		t = tokenize();
