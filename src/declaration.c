...
unsigned one_declaration(register unsigned s, unsigned type, unsigned name, unsigned defstorage)
{
	...
	if ((s == S_AUTO || s == S_REGISTER) && (defstorage == S_EXTDEF || defstorage == S_SYMDATA || defstorage == S_SYMTRANS))
		error("no automatic globals");

	if (IS_FUNCTION(type) && !PTR(type)) {
		if (s == S_EXTDEF || s == S_AUTO || s == S_SYMDATA || s == S_SYMTRANS)
			s = S_EXTERN;
	}
	...
	if (s != S_EXTERN && (PTR(type) || !IS_FUNCTION(type)) && match(T_EQ)) {
		...
		if (s == S_SYMDATA)
            header(H_SYMDATA, label, target_alignof(type, s));
		else if (s == S_SYMTRANS)
            header(H_SYMTRANS, label, target_alignof(type, s));
		else if (s >= S_LSTATIC)
            header(H_DATA, label, target_alignof(type, s));
		initializers(sym, type, s);
		if (s == S_SYMDATA)
            footer(H_SYMDATA, label, 0);
		else if (s == S_SYMTRANS)
            footer(H_SYMTRANS, label, 0);
		else if (s >= S_LSTATIC)
