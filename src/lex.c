...
void out_write(void)
{
	if (lseek(fdo, outrecord * 128L, SEEK_SET) < 0)
		fatal("seek error");
	if (outlen && write(fdo, outbuf, outlen) != outlen)
		fatal("write error");
	outlen = 0;
	outptr = outbuf;
}
...
{
	if (lseek(fdo, record * 128L, SEEK_SET) < 0)
		fatal("seek error");
	if (rread(fdo, outbuf, 128) < 0)
		fatal("read error");
	outrecord = record;
}
