...
#include "compiler.h"
#include "cc2.h"
#include "cc2-codegen.c"
#include "cc2-func.c"
#include "cc2-rewrite.c"
...
static struct node *load_tree(void)
{
	...
	xread(fdi, n, sizeof(struct node));
...
static char *getopname(int op) {
	switch(op) {
		...
		case 0x1021: return("T__DATA");
		case 0x1022: return("T__TRANSFER");
...
static void process_header(void)
{
	...
	xread(fdi, &h, sizeof(struct header));
...
//              func_ret_used = 1;
	...
	case H_SYMDATA:
		...
		if (h.h_name >= 0x8000)
			...
			gen_literal(h.h_name);
		break;
	case H_SYMDATA | H_FOOTER:
		pop_area();
		break;
	case H_SYMTRANS:
		push_area(A_SYMTRANS);
		if (h.h_name >= 0x8000)
			gen_data_label(namestr(h.h_name), h.h_data);
		else
			gen_literal(h.h_name);
		break;
	case H_SYMTRANS | H_FOOTER:
		pop_area();
		break;
	case H_STRING:
		if (h.h_data)
...
{
	uint8_t n[2];
	sym_fd = open(path, O_RDONLY | O_BINARY);
	if (sym_fd == -1) {
		perror(path);
		exit(1);
	}
