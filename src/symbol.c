...
#define S_SYMDATA	0x7000	/* SymbOS data area */
#define S_SYMTRANS	0x8000	/* SymbOS transfer area */
#define S_EXTDEF	0x9000	/* Exported global */
#define S_TYPEDEF	0xA000	/* A typedef */
#define S_STRUCT	0xB000	/* The name of a struct type */
#define S_UNION		0xC000	/* The name of a union type */
#define S_ARRAY		0xD000	/* An array description slot (unnamed) */
#define S_FUNCDEF	0xE000	/* A function definition */
#define S_ENUM		0xF000	/* An enumeration name */
			     ...
			     unsigned type)
{
	...
	if (sym != NULL && sym->type != C_ANY) {
		...
		if (symst > S_TYPEDEF)
			...
			if (symst == S_EXTERN && (storage == S_EXTDEF || storage == S_SYMDATA || storage == S_SYMTRANS)) {
				sym->infonext = S_INDEX(sym->infonext);
				sym->infonext |= storage;
				return sym;
			}
			/* foo and now found extern foo */
			if ((symst == S_EXTDEF || symst == S_SYMDATA || symst == S_SYMTRANS) && storage == S_EXTERN)
					return sym;
...
static void symbol_bss(struct symbol *s)
{
	...
	if ((PTR(t) || !IS_FUNCTION(t)) && st != S_EXTERN && st >= S_LSTATIC && st <= S_EXTDEF) {
		...
		if (!(s->infonext & INITIALIZED)) {
			...
			if (st == S_LSTATIC)
				l = s->data.offset;
            if (st == S_SYMDATA) { // we interrupt this BSS code to bring to some interstitial not-BSS wedged in here
                header(H_SYMDATA, l, target_alignof(t, st));
                put_padding_data(n);
                footer(H_SYMDATA, l, 0);
            } else if (st == S_SYMTRANS) {
                header(H_SYMTRANS, l, target_alignof(t, st));
                put_padding_data(n);
                footer(H_SYMTRANS, l, 0);
            } else { // back to your regularly scheduled programming
